...
不同格式的图集在不同的平台有不同的支持度。为了保证插件的最佳性能与最小体积，建议为不同平台提供对应格式的纹理。

各平台首选的纹理格式：

| 平台 | 推荐格式 | 说明 |
|------|---------|------|
| Windows / Linux / macOS | `.dds` (BC3) | 桌面端专用，显存占用小，加载速度快 |
| Android | `.astc` (ASTC 4x4) | 移动端专用，兼容性好 |

### 多平台纹理方案

插件作者只需维护一份插件包，在包中同时放入不同格式的纹理文件。上传到服务器后，服务端会自动按平台分离出专属插件包。
...
${entry}/
  ...
  ${entry}.lua
  _assets/images/
    texture.lua           ← 描述符，a_name 指向 ".dds" 文件
    texture.dds           ← 桌面端 (Windows/Linux/macOS)
    texture.astc          ← Android 端
  .backup/
    texture.png           ← 仅本地调试用，不上传服务器
```

> `.png` 文件不应上传，以节省网络带宽。如需本地调试，存放在 `.backup/` 目录下即可。插件管理器在上传时会自动跳过 `.backup/` 目录。

关键规则：

- **描述符**：`a_name` 需指向 `.dds` 文件（因为桌面优先用 DDS，Android 引擎会自动从 `.dds` 回落到 `.astc`）。
- **服务端拆分**：上传后服务端自动生成各平台 zip，按扩展名过滤——桌面包排除 `.astc`/`.pkm`，Android 包排除 `.dds`。
- **老插件兼容**：未包含平台特化纹理的插件不受影响，客户端下载时服务器会自动回退到完整包。
