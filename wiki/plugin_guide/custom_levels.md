...
- 自制关卡列表以**运行时已加载插件**为准：插件实现热重载接口时，点「应用」热加载后立即出现在列表中、热卸载后立即移除；未实现热重载接口的插件仍需重启后生效。
...
4. 在插件管理器中确认插件已启用（`category = "level"` 会自动归入「关卡」分类），点「应用」后即可在「自制关卡」中看到并游玩（未实现热重载接口时需重启游戏）。
5. 想分享给其他玩家：开启开发者模式（存档目录下建立 `developer.lua` 填写账号密码），在插件管理器中选中自己的插件点击「上传」；玩家即可在插件商店的「关卡」分类下载安装。

## 注意事项

- 自制关卡的进度（星星）单独记录，删除地图插件不会影响本体存档。
- 地图插件实现热重载接口时，点「应用」即可即时出现在「自制关卡」列表中（卸载则即时移除）；未实现热重载接口时，安装 / 启用后需要重启游戏。
