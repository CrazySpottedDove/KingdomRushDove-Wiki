...
- **配置延迟写盘**：在配置面板点「完成」只是记录修改，点「应用」才会真正写入 `<entry>_config.lua` 并触发 `on_config_change`。选择「直接退出」会丢弃未应用的配置修改。仅打开配置界面而未做任何修改（或改回原值）时不会产生待应用修改，关闭管理器也不会提示。
...
- **更新 / 删除正在运行的插件会直接重启**：插件文件已被替换或移除，热卸载无法撤销模板注册等全部副作用，无法安全热应用；点「应用」时若涉及正在运行的插件，直接走保存并重启。未运行的插件更新 / 删除无需重启（下次启用时加载新文件）。
...
- 关卡类插件（`category = "level"`）与其它插件行为一致：热应用后立即出现在「自制关卡」列表中，热卸载后立即移除，无需重启。
